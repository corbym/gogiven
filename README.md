# gogiven
An alternative BDD spec framework for Go. Built on top of Go's standard `go test` tool and `testing` package.

[![Build](https://github.com/corbym/gogiven/actions/workflows/build.yml/badge.svg)](https://github.com/corbym/gogiven/actions/workflows/build.yml)
...
[![Go Reference](https://pkg.go.dev/badge/github.com/corbym/gogiven.svg)](https://pkg.go.dev/github.com/corbym/gogiven)
...
2. [Example One - GoGiven in Practice](#example)
...
GoGiven is a lightweight BDD framework for producing test specifications directly from the code you write.

GoGiven parses your test file and produces a human-readable output in a specified directory, containing all the tests, captured data and other related information regarding your test such as success or failure.

GoGiven was inspired by YATSPEC, a BDD framework employed extensively by Sky Network Services (part of Sky, a UK tv company). As mentioned above, another similar product is [JGiven](http://jgiven.org).
...
func TestMyTest(t *testing.T) {
	Given(t, someData). // This is a test
	When(fooBarBaz).
	Then(baz)
}
...
Given some data noting that this is a test
When foo bar baz
Then baz 
```

Inline comments (e.g. `// This is a test`) are rendered as "Noting that …" in the output. Test data (set by the func `someData` in the example above) can be captured in a map of interesting givens, and as the test progresses, the actuals can be captured in a map of captured inputs and outputs.


### What are Interesting Givens?
...
### What are Captured Inputs and Outputs?
...
GoGiven gives you a standardised way of rendering captured data alongside your tests so you don't have to worry about it.


### Rendered how?

The test framework parses your test file, and grabs the content. It strips all non-interesting parts out and leaves the Given/When/Then format in plain text ready for a GoGivensOutputGenerator to process the text. Interesting givens and Captured inputs and outputs are maps, which are rendered alongside your test givens as table data -- interesting givens are tabulated, and captured IO is listed.
...
## Example One - GoGiven in Practice <a name="example"></a>
```go
import (
	"fmt"
	"os"
...
Table tests work the same way as normal go table tests. GoGiven will then mark which test failed, if they do, in your test output. 
...
GoGiven is configured by default with an HTML generator (`htmlspec.NewHTMLOutputGenerator`) that is consumed by a file generator (`generator.FileOutputGenerator`) (see the pkg.go.dev reference for more information). The content generator implements the following interface:
...
GoGiven's HTML spec generates an index file, which is always stored along side the test html output.

```go
func TestMain(testmain *testing.M) {
	gogiven.OutputListeners = []generator.OutputListener{new(MyFooListener)}
	// or alternately (or inclusively!)
	gogiven.OutputListeners = append(gogiven.OutputListeners, new(MyBarListener))
	runOutput := testmain.Run()
...
You can add the environment variable GOGIVENS_OUTPUT_DIR to your env properties that points to a directory you want GoGiven to report the test output to.

Default is the os's tmp directory.

## List of Pre-written Output Generators <a name="output-generator-list"></a>
